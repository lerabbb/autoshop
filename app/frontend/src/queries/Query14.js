...
export default function Query14() {
    ...
    const onSubmit= async (data) => {
        ...
        const [result] = await Promise.all([
            axios.get("http://localhost:8080/sale_content/cash-report/start="+start+"/end="+end),
        ]);
    ...
    <div className='container'>
        <div className='row'>
            <div className='col-md-6 offset-md-3 border rounded p-4 mt-2 shadow'>
                <h4 className='text-center m-4'>Получить кассовый отчет за определенный период.</h4>
      ...
      <thead>
        ...
        <tbody>{
                queryResult.map((item, index)=>(
                    <tr>
                        <th scope="row" key={item.sale.id}>{item.sale.id}</th>
                        <td>{item.stockDetail.detail.name}</td>
                        <td>{item.count}</td>
                        <td>{item.price}</td>
                        <td>{item.totalSum}</td>
                        <td>{item.sale.date}</td>
                        <td>{item.sale.consumer.lastname} {item.sale.consumer.firstname}</td>
                    </tr>
                ))}
        </tbody>
        </table>
